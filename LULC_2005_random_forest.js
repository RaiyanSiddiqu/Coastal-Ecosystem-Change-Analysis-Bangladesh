...
var col = ee.ImageCollection('LANDSAT/LT05/C02/T1_L2').filterBounds(roi)
   .filterDate('2004-11-01','2005-04-15').map(prepTM);
...
Map.addLayer(base, {bands:['Red','Green','Blue'], min:0, max:0.3}, '2005 True colour');
Map.addLayer(base, {bands:['NIR','Red','Green'], min:0, max:0.4}, '2005 False colour (veg=red)');
...
Map.addLayer(classified, {min:0,max:5,palette:palette}, 'LULC 2005');

var cm = valid.classify(rf).errorMatrix('class','classification');
print('2005 Confusion matrix:', cm);
print('2005 Overall accuracy:', cm.accuracy());
print('2005 Kappa:', cm.kappa());
print('2005 Producers:', cm.producersAccuracy());
print('2005 Users:', cm.consumersAccuracy());
...
print('2005 area by class:', ee.FeatureCollection(rows));

Export.image.toDrive({image:classified.toByte(), description:'LULC_2005', fileNamePrefix:'LULC_2005',
  region:roi, scale:30, maxPixels:1e13, crs:'EPSG:4326'});
Export.table.toDrive({collection:ee.FeatureCollection(rows), description:'LULC_2005_area',
  fileNamePrefix:'LULC_2005_area', fileFormat:'CSV'});
