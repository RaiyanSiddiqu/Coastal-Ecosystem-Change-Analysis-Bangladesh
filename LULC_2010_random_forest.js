...
// LANDSAT 5 ONLY. Widened window (Nov 2009 - Apr 2010) for enough clear scenes.
var col = ee.ImageCollection('LANDSAT/LT05/C02/T1_L2').filterBounds(roi)
   .filterDate('2009-11-01','2010-04-15').map(prepTM);

print('Number of L5 scenes found:', col.size());  // check there are enough
...
Map.addLayer(base, {bands:['Red','Green','Blue'], min:0, max:0.3}, '2010 True colour');
Map.addLayer(base, {bands:['NIR','Red','Green'], min:0, max:0.4}, '2010 False colour (veg=red)');
...
Map.addLayer(classified, {min:0,max:5,palette:palette}, 'LULC 2010');

var cm = valid.classify(rf).errorMatrix('class','classification');
print('2010 Confusion matrix:', cm);
print('2010 Overall accuracy:', cm.accuracy());
print('2010 Kappa:', cm.kappa());
print('2010 Producers:', cm.producersAccuracy());
print('2010 Users:', cm.consumersAccuracy());
...
print('2010 area by class:', ee.FeatureCollection(rows));

Export.image.toDrive({image:classified.toByte(), description:'LULC_2010', fileNamePrefix:'LULC_2010',
  region:roi, scale:30, maxPixels:1e13, crs:'EPSG:4326'});
Export.table.toDrive({collection:ee.FeatureCollection(rows), description:'LULC_2010_area',
  fileNamePrefix:'LULC_2010_area', fileFormat:'CSV'});
