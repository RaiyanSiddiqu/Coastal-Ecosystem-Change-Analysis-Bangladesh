...
// WIDER window for 1995 (fewer scenes available historically)
var col = ee.ImageCollection('LANDSAT/LT05/C02/T1_L2').filterBounds(roi)
   .filterDate('1994-10-15','1995-04-30').map(prepTM);
print('Number of L5 scenes found:', col.size());  // if <5, widen window further
...
Map.addLayer(base, {bands:['Red','Green','Blue'], min:0, max:0.3}, '1995 True colour');
Map.addLayer(base, {bands:['NIR','Red','Green'], min:0, max:0.4}, '1995 False colour (veg=red)');
...
Map.addLayer(classified, {min:0,max:5,palette:palette}, 'LULC 1995');

var cm = valid.classify(rf).errorMatrix('class','classification');
print('1995 Confusion matrix:', cm);
print('1995 Overall accuracy:', cm.accuracy());
print('1995 Kappa:', cm.kappa());
print('1995 Producers:', cm.producersAccuracy());
print('1995 Users:', cm.consumersAccuracy());
...
print('1995 area by class:', ee.FeatureCollection(rows));

Export.image.toDrive({image:classified.toByte(), description:'LULC_1995', fileNamePrefix:'LULC_1995',
  region:roi, scale:30, maxPixels:1e13, crs:'EPSG:4326'});
Export.table.toDrive({collection:ee.FeatureCollection(rows), description:'LULC_1995_area',
  fileNamePrefix:'LULC_1995_area', fileFormat:'CSV'});
