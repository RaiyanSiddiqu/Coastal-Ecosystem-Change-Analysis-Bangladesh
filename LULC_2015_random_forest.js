...
var col = ee.ImageCollection('LANDSAT/LC08/C02/T1_L2').filterBounds(roi)
   .filterDate('2014-11-01','2015-03-01').map(prepOLI);
...
Map.addLayer(base, {bands:['Red','Green','Blue'], min:0, max:0.3}, '2015 True colour');
Map.addLayer(base, {bands:['NIR','Red','Green'], min:0, max:0.4}, '2015 False colour (veg=red)');
Map.addLayer(mndwi, {min:-0.5,max:0.5,palette:['white','blue']}, 'MNDWI (water bright)');

// >>> Draw builtup, agri, water, aqua, veg, bare (FeatureCollections). Then uncomment.
...
Map.addLayer(classified, {min:0,max:5,palette:palette}, 'LULC 2015');

var cm = valid.classify(rf).errorMatrix('class','classification');
print('2015 Confusion matrix:', cm);
print('2015 Overall accuracy:', cm.accuracy());
print('2015 Kappa:', cm.kappa());
print('2015 Producers:', cm.producersAccuracy());
print('2015 Users:', cm.consumersAccuracy());
...
print('2015 area by class:', ee.FeatureCollection(rows));

Export.image.toDrive({image:classified.toByte(), description:'LULC_2015', fileNamePrefix:'LULC_2015',
  region:roi, scale:30, maxPixels:1e13, crs:'EPSG:4326'});
Export.table.toDrive({collection:ee.FeatureCollection(rows), description:'LULC_2015_area',
  fileNamePrefix:'LULC_2015_area', fileFormat:'CSV'});
