...
// 2025 dry season (Nov 2024 - Mar 2025). Landsat 8 + 9 (OLI).
...
// spectral indices (help separate water / aquaculture / veg / built / bare)
...
// display layers to help you label training data
Map.addLayer(base, {bands:['Red','Green','Blue'], min:0, max:0.3}, '2025 True colour');
Map.addLayer(base, {bands:['NIR','Red','Green'], min:0, max:0.4}, '2025 False colour (veg=red)');
Map.addLayer(mndwi, {min:-0.5,max:0.5,palette:['white','blue']}, 'MNDWI (water bright)');
// ------------------------- 2. TRAINING DATA --------------------------------
// >>> DRAW SIX geometry imports with the geometry tools, named EXACTLY:
//     builtup, agri, water, aqua, veg, bare
//     For each: set it as a FeatureCollection and add property 'class' =
//     builtup->0  agri->1  water->2  aqua->3  veg->4  bare->5
//     Draw ~12-15 polygons per class, spread across the area (more for
//     aquaculture & agriculture). THEN uncomment everything below and run.
...
//             builtup    agri    water    aqua    veg     bare
Map.addLayer(classified, {min:0,max:5,palette:palette}, 'LULC 2025');
 
// ------------------------- 4. ACCURACY -------------------------------------
var vc = valid.classify(rf);
var cm = vc.errorMatrix('class','classification');
print('Confusion matrix (2025):', cm);
print('Overall accuracy:', cm.accuracy());
print('Kappa:', cm.kappa());
print('Producers accuracy:', cm.producersAccuracy());
print('Users accuracy:', cm.consumersAccuracy());
 
// ------------------------- 5. AREA PER CLASS (ha + km2) --------------------
var areaImg = ee.Image.pixelArea().divide(10000).addBands(classified); // ha
var areas = areaImg.reduceRegion({
  reducer: ee.Reducer.sum().group({groupField:1, groupName:'class'}),
  geometry: roi, scale: 30, maxPixels: 1e13, tileScale: 4
});
var classNames = ee.Dictionary({
  '0':'Built-up','1':'Agriculture','2':'Waterbody',
  '3':'Aquaculture','4':'Vegetation','5':'Bareland'
});
var groups = ee.List(areas.get('groups'));
var rows = groups.map(function(g){
  g = ee.Dictionary(g);
  var cls = ee.Number(g.get('class')).format('%d');
  var ha  = ee.Number(g.get('sum'));
  return ee.Feature(null, {
    class_id: cls, class_name: classNames.get(cls),
    area_ha: ha.round(), area_km2: ha.divide(100).round()
  });
});
var areaFC = ee.FeatureCollection(rows);
print('LULC 2025 area by class (ha & km2):', areaFC);
 
// ------------------------- 6. EXPORTS --------------------------------------
Export.image.toDrive({
  image: classified.toByte(), description: 'LULC_2025', fileNamePrefix: 'LULC_2025',
  region: roi, scale: 30, maxPixels: 1e13, crs: 'EPSG:4326'
});
Export.table.toDrive({
  collection: areaFC, description: 'LULC_2025_area_by_class',
  fileNamePrefix: 'LULC_2025_area_by_class', fileFormat: 'CSV'
});